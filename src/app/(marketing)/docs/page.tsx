    ...
    <>
      ...
      <div className="mx-auto flex max-w-[1180px] items-start gap-12 px-6">
        <aside className="sticky top-[76px] hidden w-[210px] flex-none py-12 lg:block">
          <p className="mb-3 font-jetbrains text-[11px] uppercase tracking-wider text-muted-foreground">
            En esta página
          </p>
          <nav className="flex flex-col gap-1">
            {DOCS_NAV.map((n) => (
              <a
                key={n.href}
                href={n.href}
                className="rounded-md px-2 py-1.5 text-sm text-muted-foreground transition-colors hover:bg-foreground/10 hover:text-foreground"
              >
                {n.label}
              </a>
            ))}
