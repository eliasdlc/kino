...
import type { TodayCheckinRowTransport, WeeklyTrend, LearningInsight } from "@/features/energy/energy.types";
...
/**
 * Un id de muestra. `Id<never>` es asignable a cualquier `Id<tabla>`, así que
 * el mismo valor sirve donde el componente pide una tarea, una página o un
 * sistema, sin un cast por sitio.
 */
export const mid = (value: string | number): Id<never> =>
  (typeof value === "number" ? `00000000-0000-4000-8000-${String(value).padStart(12, "0")}` : value) as Id<never>;
const uuid = mid;
...
export function mockLearningInsight(overrides: Partial<LearningInsight> = {}): LearningInsight {
  ...
  return { ...base, ...overrides } as LearningInsight;
}
