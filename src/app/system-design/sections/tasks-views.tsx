...
export function TasksViewsSection() {
  ...
  const sprints = [
    makeSprint(),
    makeSprint({ id: "spr-2", name: "Sprint 2", status: "completed" }),
  ];
    ...
    >
      ...
      >
        <div className="max-w-2xl space-y-1">
          ...
          />
          <TaskListRow
            task={makeTask({ id: "r2", title: "Fila enfocada (navegación con teclado)" })}
            systemMap={SYSTEM_MAP}
            onToggle={noop}
            onOpen={noop}
            isFocused
          />
          <TaskListRow
            task={makeTask({ id: "r3", title: "Fila seleccionada (bulk)" })}
            systemMap={SYSTEM_MAP}
            onToggle={noop}
            onOpen={noop}
            isSelected
          ...
          />
          <TaskListRow
            task={makeTask({ id: "r4", title: "Completada", status: "done" })}
            systemMap={SYSTEM_MAP}
            onToggle={noop}
            onOpen={noop}
          />
      ...
      >
        <div className="max-w-2xl">
          <OverdueGroup
            tasks={[
              makeTask({ id: "o1", title: "Entregar práctica 4", dueDate: daysFromNow(-2) }),
              makeTask({ id: "o2", title: "Pagar matrícula", dueDate: daysFromNow(-5), priority: "critical" }),
            ]}
            systemMap={SYSTEM_MAP}
            onOpen={noop}
            onToggle={noop}
      ...
      >
        <div className="grid max-w-2xl grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          />
          <PlanningTaskCard
            task={makeTask({ id: "pl2", title: "Completada", status: "done" })}
            onToggle={noop}
            onDelete={noop}
            onEdit={noop}
          />
      ...
      >
        <div className="grid max-w-2xl grid-cols-7 gap-y-1 rounded-lg border border-dashed border-border p-3">
          ...
          />
          <MultiDayTaskBar
            task={makeTask({ id: "m2", title: "Viaje", priority: "low" })}
            startCol={4}
            span={3}
          />
        </div>
