...
/**
 * Sólo se limita lo caro: MCP y las mutaciones. Las lecturas pasan sin
 * roundtrip añadido, que es la condición bajo la que la vía Postgres sale a
...
export function policyFor(pathname: string, method: string): RateLimitPolicy | null {
  ...
  if (pathname.startsWith('/api/auth/') || pathname.startsWith('/api/cron/')) return null;
