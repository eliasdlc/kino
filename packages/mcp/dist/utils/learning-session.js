...
function checkpointBlock(checkpoint) {
    return ['```json', JSON.stringify(checkpoint, null, 2), '```'].join('\n');
}
export function buildCheckpoint(sessionId, draft, at) {
    return {
        schemaVersion: 1,
        sessionId,
...
function sectionBounds(lines, heading) {
    ...
    if (start === -1) {
        throw new Error(`Esta página no es una sesión de aprendizaje de Kino: le falta la sección "${heading}".`);
    }
    let end = start + 1;
...
export function readCheckpoint(markdown) {
    if (!markdown?.trim()) {
    ...
    if (!result.success) {
        throw new Error(`El checkpoint de la sesión no tiene la forma esperada: ${result.error.issues
...
export function writeSession(markdown, input) {
 ...
 */
export function appendInteraction(markdown, entry) {
    const lines = markdown.split('\n');
