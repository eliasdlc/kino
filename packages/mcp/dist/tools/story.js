...
const entityType = z
    ...
    .describe('Tipo de entidad: character (personaje), location (lugar), object (objeto), concept (concepto/lore), event (evento), faction (facción), other');
const asText = (data) => ({
    content: [{ type: 'text', text: JSON.stringify(data, null, 2) }],
});
export function registerStoryTools(server, kinoFetch) {
    server.tool('list_entities', 'Lista el codex de un sistema de escritura: todos los personajes, lugares, objetos y conceptos del universo, con sus alias y resumen', {
    ...
    server.tool('get_entity', 'Obtiene la ficha completa de una entidad del codex: atributos, relaciones y en qué capítulos aparece (con la primera aparición)', {
        entityId: z.string().uuid().describe('UUID de la entidad'),
    }, async ({ entityId }) => asText(await kinoFetch(`/api/entities/${entityId}`)));
    server.tool('create_entity', 'Crea una entidad en el codex de un sistema de escritura (personaje, lugar, objeto…)', {
        systemId: z.string().uuid().describe('UUID del sistema de escritura'),
    ...
    server.tool('update_entity', 'Actualiza una entidad del codex: nombre, tipo, alias, resumen o atributos', {
        entityId: z.string().uuid().describe('UUID de la entidad'),
    ...
    }, async ({ entityId, ...body }) => asText(await kinoFetch(`/api/entities/${entityId}`, {
        method: 'PATCH',
        body: JSON.stringify(body),
    })));
    server.tool('link_entities', 'Crea una relación entre dos entidades del codex ("padre de", "rival de", "vive en"). La dirección va de fromEntityId a toEntityId', {
    ...
    }, async ({ fromEntityId, ...body }) => asText(await kinoFetch(`/api/entities/${fromEntityId}/relations`, {
        method: 'POST',
        body: JSON.stringify(body),
    })));
    ...
    }, async ({ systemId, query }) => asText(await kinoFetch(`/api/systems/${systemId}/story-search?q=${encodeURIComponent(query)}`)));
}
