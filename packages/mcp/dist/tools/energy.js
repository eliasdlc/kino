...
export function registerEnergyTools(server, kinoFetch) {
    ...
    server.tool('create_energy_checkin', 'Registra el check-in de energía del usuario para hoy. currentLevel es 1-100 y sleepQuality es good/partial/poor.', {
        currentLevel: z
            .number()
            .int()
            .min(1)
    ...
    server.tool('get_today_plan', 'Obtiene el plan de energía de hoy: tareas recomendadas ajustadas al nivel de energía y límite diario del usuario.', {}, async () => {
        const plan = await kinoFetch('/api/energy/plan/today');
        return { content: [{ type: 'text', text: JSON.stringify(plan, null, 2) }] };
    });
    // ── Time-blocking consciente de energía ──────────────────────────────────
    server.tool('get_energy_windows', 'Ventanas de energía del usuario y presupuesto del día: cronotipo, curva (aprendida o teórica), ventana pico, capacidad media por tramo (mañana/tarde/noche), predicción guardada de hoy y energía comprometida vs. límite. Úsalo ANTES de proponer o mover bloques. Si peak es null, Kino todavía no aprendió la curva: no inventes una mejor ventana.', {}, async () => {
        const windows = await kinoFetch('/api/energy/windows');
        return { content: [{ type: 'text', text: JSON.stringify(windows, null, 2) }] };
    });
    server.tool('propose_day_blocks', 'Propone bloques horarios para un día usando el mismo planificador que ve el usuario: coloca lo exigente en los tramos de más energía, sugiere pausas cada ~90 min y explica por qué cada tarea va donde va. NO escribe nada — devuelve la propuesta y las tareas que quedaron fuera con su motivo (budget = no cabe en el día, energy = pide más energía de la proyectada). Para escribirla usa schedule_task_block.', {
        date: z
            .string()
            .regex(/^\d{4}-\d{2}-\d{2}$/)
            .optional()
