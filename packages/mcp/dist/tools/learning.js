...
import { z } from 'zod';
import { parseLearningCheckpoint, renderLearningSessionDocument, updateLearningSessionDocument, } from '../utils/learning-session.js';
...
function checkpointFromInput(sessionId, input) {
    return {
        schemaVersion: 1,
        sessionId,
...
export function registerLearningTools(server, kinoFetch) {
    server.tool('create_learning_session', 'Crea una sesión de aprendizaje reanudable en una página de Kino', {
    ...
    server.tool('get_learning_session', 'Recupera una sesión de aprendizaje y su checkpoint listo para reanudar', {
        pageId: z.string().uuid().describe('UUID de la página de sesión'),
    }, async ({ pageId }) => {
        const page = pageAsMarkdown(await kinoFetch(`/api/pages/${pageId}`));
        if (!page.content)
            throw new Error('The learning session page has no content.');
        const checkpoint = parseLearningCheckpoint(page.content);
        return {
            content: [{ type: 'text', text: JSON.stringify({ page, checkpoint }, null, 2) }],
        };
    });
    server.tool('save_learning_checkpoint', 'Actualiza Ahora, Por qué, Después y el checkpoint sin pisar una edición más reciente', {
        pageId: z.string().uuid().describe('UUID de la página de sesión'),
    ...
    server.tool('append_learning_interaction', 'Registra una interacción de aprendizaje sin duplicarla si la página ya cambió', {
        pageId: z.string().uuid().describe('UUID de la página de sesión'),
    ...
    server.tool('park_learning_thought', 'Guarda una distracción o idea en Kino y devuelve el mismo punto de aprendizaje para continuar', {
        pageId: z.string().uuid().describe('UUID de la página de sesión'),
