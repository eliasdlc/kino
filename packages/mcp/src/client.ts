export type KinoFetch = <T = unknown>(
  path: string,
  options?: RequestInit,
) => Promise<T>;
 ...
 */
export function createKinoFetch({
  baseUrl,
  token,
}: {
  baseUrl: string;
  ...
  return async function kinoFetch<T = unknown>(
    path: string,
    options: RequestInit = {},
  ): Promise<T> {
    ...
    if (res.status === 401) {
      throw new Error(
        'Kino: authentication failed. Your API key or access token is invalid or has been revoked.',
      );
    }
    if (res.status === 429) {
