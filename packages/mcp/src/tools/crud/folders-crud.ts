...
export function registerFolderCrudTools(server: McpServer) {
  ...
  server.tool(
    'get_folder_children',
    'Lista las subcarpetas directas de una carpeta padre en Kino',
    {
  ...
  server.tool(
    'list_folder_tasks',
    'Lista las tareas asignadas directamente a una carpeta de un sistema',
    {
      systemId: z.string().uuid().describe('UUID del sistema'),
  ...
  server.tool(
    'create_folder',
    'Crea una carpeta en un sistema de Kino. Puede anidarse bajo otra carpeta con parentId.',
    {
  ...
  server.tool(
    'update_folder',
    'Actualiza el nombre o color de una carpeta en Kino',
    {
