...
export function registerPageCrudTools(server: McpServer, kinoFetch: KinoFetch) {
  server.tool(
    'list_pages',
  ...
  server.tool(
    'append_page_content',
    'Añade Markdown al final de una página sin sobrescribir cambios hechos después de la última lectura',
    {
      pageId: z.string().uuid().describe('UUID de la página'),
  ...
  server.tool(
    'list_page_tasks',
    'Lista las tareas vinculadas a una página de Kino',
    {
      pageId: z.string().uuid().describe('UUID de la página'),
    },
    async ({ pageId }) => {
  ...
  server.tool(
    'link_task_to_page',
    'Vincula una tarea existente a una página de Kino (relación de referencia, no cambia la ubicación de la tarea)',
    {
      pageId: z.string().uuid().describe('UUID de la página'),
  ...
  server.tool(
    ...
    'Desvincula una tarea de una página de Kino (no elimina la tarea)',
    {
      pageId: z.string().uuid().describe('UUID de la página'),
