...
export function registerTaskBulkTools(server: McpServer) {
  ...
  server.tool(
    'bulk_update_tasks',
    'Actualiza la prioridad de múltiples tareas en una sola operación (máximo 50)',
    {
  ...
  server.tool(
    'restore_task',
    'Restaura una tarea previamente eliminada (soft-delete) en Kino, devolviéndola a la papelera al estado activo.',
    {
  ...
  server.tool(
    'log_task_time',
    'Registra una sesión de tiempo trabajada en una tarea (time log). Las fechas van en ISO 8601 UTC.',
    {
