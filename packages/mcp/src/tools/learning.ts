...
const versionInput = z.iso
  ...
  .describe(
    'El `updatedAt` que devolvió la última lectura o escritura de esta sesión. Si la página cambió desde entonces, la llamada falla con 409 en vez de pisar lo que haya: vuelve a leerla y reintenta sobre lo nuevo',
  );
...
async function readSession(kinoFetch: KinoFetch, pageId: string) {
...
async function saveSession(
  ...
  const saved = storedPage.parse(
    await kinoFetch(`/api/pages/${pageId}`, {
      method: 'PATCH',
...
export function registerLearningTools(server: McpServer, kinoFetch: KinoFetch) {
  server.tool(
    ...
    async ({ systemId, folderId, topic, now, why, ...draft }) => {
      ...
      const page = storedPage.parse(
        await kinoFetch('/api/pages', {
          method: 'POST',
          body: JSON.stringify({
            systemId,
            folderId,
  ...
  server.tool(
    ...
    async ({ pageId, expectedUpdatedAt, now, why, ...draft }) => {
      ...
      const updatedAt = await saveSession(
        kinoFetch,
        pageId,
        writeSession(markdown!, { now, why, checkpoint }),
        expectedUpdatedAt,
      );
  ...
  server.tool(
    ...
    async ({ pageId, expectedUpdatedAt, kind, content, recordedAt }) => {
      ...
      const updatedAt = await saveSession(
        kinoFetch,
        pageId,
        appendInteraction(markdown!, { kind, content, at }),
        expectedUpdatedAt,
      );
