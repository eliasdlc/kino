...
export function renderLearningSessionDocument({
  topic,
  now,
  why,
  checkpoint,
}: LearningSessionDocument): string {
  return `# ${topic.trim()}
...
export function parseLearningCheckpoint(markdown: string): ResumeCheckpoint {
...
export function updateLearningSessionDocument(
  markdown: string,
  input: {
    now: string;
    why: string;
